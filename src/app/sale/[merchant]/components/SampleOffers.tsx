...
function OfferCard({ offer }: { offer: SampleOffer }) {
  ...
  return (
    <motion.div
      ...
      className="group flex h-full flex-col rounded-2xl border border-white/10 bg-gradient-to-b from-white/[0.06] to-white/[0.02] p-5 transition-colors hover:border-primary-orange/30"
    >
      ...
      <ul className="mt-4 flex-1 space-y-2">
        {offer.bullets.map((b) => (
          <li key={b} className="flex items-start gap-2 text-sm text-white/75">
            <Check className="mt-0.5 size-4 shrink-0 text-primary-success" />
            {b}
          </li>
        ))}
      </ul>

      <p className="mt-4 text-xs text-white/40">{offer.note}</p>

      <Link
        href="/sign-in"
        data-cta-source="sample-offer"
        className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-primary-orange transition-colors hover:text-primary-orange/80"
      >
        Learn more
