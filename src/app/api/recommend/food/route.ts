...
import { foodRecommendInputSchema } from "@/schemas/advisor";
...
export async function POST(req: Request) {
  ...
  const parsed = foodRecommendInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    ...
    const result = recommendFoodCardPhaseTwo(
      parsed.data,
      AdvisorCache.cards(),
      AdvisorCache.bestOf(),
      await AdvisorCache.rulesForEngine("food"),
      {
        profile: {
    ...
    console.error("[/api/recommend/food] failed:", err);
