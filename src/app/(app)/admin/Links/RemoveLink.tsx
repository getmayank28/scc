...
import { useDeleteLinkMutation } from "@/store/admin";
...
const RemoveLink = ({ open, onChange, link }: RemoveLinkProps) => {
  const [deleteLink, { isLoading }] = useDeleteLinkMutation();

  const handleRemoveLink = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  return (
    ...
    >
      <Typography
        variant="body"
        className="opacity-100 text-sm font-semibold mb-5"
      >
        Are you sure you want to remove{" "}
        <span className="text-red-400">{link?.name}</span>?
      </Typography>
