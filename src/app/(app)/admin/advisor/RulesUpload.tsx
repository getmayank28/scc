...
const REASON_LABELS: Record<string, string> = {
  ...
  bad_voucher_cap: "Bad voucher_cap (value/period/metric/scope)",
...
const COLUMNS: { name: string; required: boolean; desc: string }[] = [
  ...
  { name: "voucher_shared_cap_group", required: false, desc: 'Voucher-lane shared group, same token format as shared_cap_group (e.g. 10:smartbuy_e_vouchers:combined). Pools the members\' voucher_cap across rules on the card; never merges with the direct-lane pool even with the same token. Members must set the same voucher_cap. Blank = the voucher_cap (if any) is private to this rule.' },
  { name: "voucher_cap_period", required: false, desc: "daily / monthly / quarterly / annually. Defaults to monthly when a voucher_cap_value is set." },
  { name: "voucher_cap_metric", required: false, desc: 'purchase_inr / points / inr / cashback. Defaults to purchase_inr (caps the ₹ of vouchers bought per period; replaces the deprecated voucher_monthly_purchase_limit_inr). A reward metric instead caps the voucher lane\'s reward value, replacing reward_cap in that lane.' },
  { name: "voucher_cap_value", required: false, desc: "The voucher cap amount. The voucher cap is only created when this is set; blank = voucher lane behaves as before (rewards budget against reward_cap, no purchase limit)." },
  { name: "voucher_cap_scope", required: false, desc: "merchant / category / card. Defaults to merchant when a voucher_cap_value is set." },
