...
const CardRecommendationModal = ({ open = false, onClose, ...rest }: BotRecommendationCreditCardProps & { open?: boolean, onClose: () => void }) => {
    return (
        <Modal
            ...
            className="relative h-fit min-h-[65vh] border-2 border-brown-border  bg-brown-background w-[800px] min-w-[800px] max-w-[80vw]"
        >
            <div onClick={onClose} className="z-40 absolute top-5 cursor-pointer right-5 bg-[#372921] w-10 h-10 rounded-full flex justify-center items-center"><X color="#ffffff"/></div>
            <div>
            ...
            <div className="bg-[#372921] flex justify-between p-4 absolute bottom-0 left-0 w-full">
                <div className="bg-brown-sidebar text-[12px] p-2 px-4 rounded-full text-white/80 font-bold uppercase">Annual Fee: {rest?.annualFee}</div>
                <Button className="font-bold" onClick={()=>window.open(rest?.applyLink, '_blank')}>Apply for this card</Button>
            </div>
        </Modal>
    )
