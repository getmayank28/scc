...
export default function CreditCardComparison() {
  const [wrongCardSpent, setWrongCardSpent] = useState(0);
  const [missedRewards, setMissedRewards] = useState<Array<RewardProps>>([]);

  useEffect(() => {
    const interval = setInterval(() => {
      setWrongCardSpent((prev) => (prev <= 100000 ? prev + 100 : 0));

      if (Math.random() > 0.6) {
        setMissedRewards((prev) => [
          ...prev.slice(-8),
          {
        ...
        <div className="relative bg-transparent overflow-hidden mb-8">
          <div className="mx-auto rounded-xl w-[440px] h-[260px] max-md:w-[342px] max-md:h-[232px]  border border-destructive/80 flex justify-center bg-transparent relative overflow-hidden">
            <div className="w-[440px] h-[260px]">
                ...
                >
                  ...
                  <defs>
                    ...
                    >
                      ...
                      <feColorMatrix
                        type="matrix"
                        values="0 0 0 0 0.604232 0 0 0 0 0.604232 0 0 0 0 0.604232 0 0 0 1 0"
                      />
                      <feBlend
                        mode="normal"
                        in2="BackgroundImageFix"
                        result="effect1_dropShadow_2195_891"
                      />
                      <feBlend
                        mode="normal"
                        in="SourceGraphic"
                        in2="effect1_dropShadow_2195_891"
                        result="shape"
                      />
                    </filter>
                    <linearGradient
                      id="paint0_linear_2195_891"
                      x1="40.5"
                      y1="0.5"
                      x2="0.999997"
                      y2="31"
              ...
              <div className="absolute top-1/2 left-4 transform -translate-y-1/2 text-white text-xl font-mono font-bold opacity-100 tracking-wider">
                ₹{wrongCardSpent.toLocaleString()}
              </div>
              <div className="absolute top-1/2 left-34 transform -translate-y-1/2 text-destructive text-xl font-mono font-bold opacity-100 tracking-wider">
                ₹{(wrongCardSpent * 0.02).toFixed(0)}
              </div>
              ...
              <div className="absolute top-[61%] -translate-y-1/2 left-34 flex justify-between items-end text-destructive opacity-75">
                You&apos;ve lost
              </div>
            </div>
          </div>
          ...
          <div className="absolute h-full w-full top-0 left-0 z-30">
            <div className="relative h-full rounded-2xl  overflow-hidden">
              {/* Floating missed rewards */}
              {missedRewards.map((reward) => (
                <div
                  key={reward.id}
                  className="absolute animate-float-away"
                  style={{ left: `${reward.x}%`, bottom: "20%" }}
                >
                  <div className="bg-destructive text-white px-4 py-2 rounded-full font-bold text-sm whitespace-nowrap shadow-lg">
                    -₹{reward.amount * 90}
                  </div>
                </div>
              ))}
