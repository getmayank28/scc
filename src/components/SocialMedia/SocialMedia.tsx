...
export default function SocialMediaIcons({landing}:{landing?:boolean}) {
  return (
    <div className="flex flex-col items-start gap-6">
     {!landing&& <div className='flex flex-col justify-start gap-3'>
      ...
      <ul className={`flex gap-5 ${landing?"mt-5":""}`}>
          ...
          return (
            <li key={index} className="list-none">
              <a
                ...
                target='_blank'
                className={`relative group transition-all duration-500 hover:border-primary-orange flex ${landing?"h-8 w-8 border border-white/30":"h-12 w-12 border-2 border-white/50 rounded-full"}  items-center justify-center overflow-hidden   bg-transparent`}
              >
                <Icon className={`relative z-[3] ${landing?"h-4 w-4":"h-6 w-6"}  transition-all duration-500 text-[#fff] group-hover:text-primary-orange`} />
              </a>
            </li>
          )
        })}
      </ul>
      {!landing&& <div className='mt-auto'>
